...
export default function Blog() {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center bg-gray-900 min-h-screen">
      <div
      ...
      <div className="my-2 h-full p-6 md:p-10 w-full md:w-2/3 lg:w-1/2">
        <div className="mb-10 relative">
          <input
            ...
            className="w-full bg-transparent border focus:outline-none text-gray-200 p-3 pr-10e text-lg placeholder-gray-500"
            onChange={filterBlogs}
          />
          <FaSearch className="text-gray-200 text-2xl absolute right-0 top-1/2 transform -translate-x-1/2 -translate-y-1/2" />
        </div>
        {posts.map((post, key) => {
          return (
            <Link
              to={"/blog/" + post.internallink}
              className="py-8 cursor-pointer flex flex-col transition-none mb-5 border-t border-gray-700 group"
              key={key}
            >
              <h1 className="font-bold text-xl xl:text-2xl text-white group-hover:text-secondaryDark">
                {post.title}
              </h1>
        ...
        {posts.length === 0 && (
          <div className="w-full font-bold text-xl xl:text-2xl text-white group-hover:text-secondaryDark">
